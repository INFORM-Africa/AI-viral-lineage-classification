...
def load_data(file_name):
    ...
    y_test_orig = data[data['Train'] == 2]['Target']  # Retain original targets for test set

    return X_train, y_train, X_test, y_test_orig, label_encoder

def predict_and_save(X_train, y_train, X_test, y_test_orig, label_encoder, params, file_name):
    """
    ...
    predictions_encoded = rf.predict(X_test)
    predictions = label_encoder.inverse_transform(predictions_encoded)

    # File path for the predictions CSV
    predictions_file_path = '../predictions/random_forest.csv'
    ...
    if os.path.exists(predictions_file_path):
        ...
        predictions_df['Original_Targets'] = y_test_orig.values  # Add original targets to the CSV

    # Add the new predictions as a column
    predictions_df[file_name] = predictions
...
def main():
    ...
    parser.add_argument('--criterion', type=str, default='gini', choices=['gini', 'entropy'], help='The function to measure the quality of a split.')
    parser.add_argument('--class_weight', type=str, default=None, choices=[None, 'balanced'], help='Weights associated with classes.')
    ...
    X_train, y_train, X_test, y_test_orig, label_encoder = load_data(file_name)
    predict_and_save(X_train, y_train, X_test, y_test_orig, label_encoder, params, file_name)
