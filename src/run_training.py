#!/usr/bin/env python3
"""
AI Viral Lineage Classification - Training Runner
...
Usage:
    python run_training.py path/to/config.toml
"""
...
def build_training_args(config: Dict[str, Any], dataset_basename: str) -> list:
    """
    ...
    args = [
        ...
        '--low_resource', str(config.get('LOW_RESOURCE', 0)),
        '--num_features', str(config.get('NUM_FEATURES', 1000))
    ]
...
def build_segment_training_args(config: Dict[str, Any], dataset_basename: str) -> list:
    """
    Build the command line arguments for segment training.
...
def run_training(config: Dict[str, Any]) -> None:
    ...
    if segment_enabled == 1:
        script_name = 'train_segment.py'
    elif dask_enabled == 1:
        script_name = 'train_dask.py'
    else:
    ...
    # Build command line arguments
    if segment_enabled == 1:
        training_args = build_segment_training_args(config, dataset_basename)
    else:
        training_args = build_training_args(config, dataset_basename)
    ...
    try:
        cmd = [sys.executable, str(script_path)] + training_args
        print(f"Command: {' '.join(cmd)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='AI Viral Lineage Classification - Training Runner',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    python run_training.py settings/dengue_standard.toml
    python run_training.py my_custom_config.toml
        """
    )
    ...
    if segment_enabled == 1:
        ...
        # Run segment training directly
        print("\n" + "="*50)
        run_training(config)
    else:
        # Run normal pipeline: preprocessing -> feature extraction -> training
        print("\n" + "="*50)
