...
def main():
    ...
    output_filename = f'{output_dir}/ACS_{args.Degenerate}.parquet'
    
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    acs_data.columns = acs_data.columns.map(str)
    acs_data.to_parquet(output_filename, engine='pyarrow')
    
    print(f"ACS features saved to {output_filename}")
