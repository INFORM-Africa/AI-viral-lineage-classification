...
def main():
    ...
    dsp_data = np.zeros((len(data), median_length), dtype=np.float16)
    for i in tqdm(range(len(data)), desc="Processing sequences"):
        dsp_data[i] = process_sequence(data["Sequence"][i], median_length, mapping)  # Assuming 'mapping' is defined elsewhere
    ...
    output_filename = f'{output_dir}/DSP_{args.Degenerate}_{args.Numeric.lower()}.parquet'
    
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    results.columns = results.columns.map(str)
    results.to_parquet(output_filename, engine='pyarrow')
    print(f"DSP features saved to {output_filename}")
