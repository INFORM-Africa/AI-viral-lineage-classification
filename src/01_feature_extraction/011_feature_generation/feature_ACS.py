...
def main():
    ...
    comparison_sequences = random_undersample(sequence_df)["Sequence"]
    ...
    def calculate_acs_for_genome(genome):
        ...
        for i, comp_seq in enumerate(comparison_sequences):
            scores[i] = calculate_ACS(genome, comp_seq)  # Assuming 'acs' is a previously defined function
        return scores
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        futures = {executor.submit(calculate_acs_for_genome, genome): genome for genome in sequence_df["Sequence"]}
        ...
        for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="Computing ACS distances"):
            ACS_features.append(future.result())

    targets = sequence_df['Target'].to_list()
    train_labels = sequence_df['Train'].to_list()
    save_features_to_parquet(ACS_features, args, targets, train_labels)
