...
class LocalClassifierPerLevel(BaseEstimator, HierarchicalClassifier):
    ...
    def __init__(
        ...
        Parameters local_classifier (BaseEstimator, default=LogisticRegression) : The local_classifier used to create
        the collection of local classifiers. Needs to have fit, predict and clone methods.

            verbose (int, default=0) : Controls the verbosity when fitting and predicting. See
            https://verboselogs.readthedocs.io/en/latest/readme.html#overview-of-logging-levels for more information.

            edge_list (str, default=None) : Path to write the hierarchy built.

            replace_classifiers (bool, default=True) : Turns on (True) the replacement of a local classifier with a
            constant classifier when trained on only a single unique class.

            n_jobs (int, default=1) : The number of jobs to run in parallel. Only :code:`fit` is parallelized. If
            :code:`Ray` is installed it is used, otherwise it defaults to :code:`Joblib`.

            bert (bool, default=False) : Whether to skip scikit-learn's checks and sample_weight passing for BERT.

            tmp_dir (str, default=None) : Temporary directory to persist local classifiers that are trained. If the
            job needs to be restarted, it will skip the pre-trained local classifier found in the temporary directory.
        """
        super().__init__(
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        Parameters
            ...
            Internally, its dtype will be converted to ``dtype=np.float32``. If a sparse matrix is provided,
            it will be converted into a sparse ``csc_matrix``.

            y (array-like of shape (n_samples, n_levels)) : The target values, i.e., hierarchical class labels for
            classification. sample_weight (array-like of shape ( n_samples,), default=None) : Array of weights that
            are assigned to individual samples. If not provided, then each sample is given unit weight.
    ...
    def predict(self, X):
        ...
        Parameters
            ...
            X ({array-like, sparse matrix} of shape (n_samples, n_features)) : The input samples. Internally,
            its dtype will be converted to ``dtype=np.float32``. If a sparse matrix is provided, it will be converted
            into a sparse ``csr_matrix``. Returns y (ndarray of shape (n_samples,) or (n_samples, n_outputs)) : The
            predicted classes.
        """
