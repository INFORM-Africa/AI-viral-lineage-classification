...
class Explainer:
    ...
    def __init__(
        ...
        Parameters
            ...
            algorithm (str, default="auto") : The algorithm to use for SHAP explainer. 
                Possible values are 'linear', 'tree', 'auto', 'permutation', or 'partition'
            mode( str, default="") : The mode of the SHAP explainer. 
                Can be 'tree', 'gradient', 'deep', 'linear', or '' for default SHAP explainer.

        Examples
            >>> from sklearn.ensemble import RandomForestClassifier
            >>> import numpy as np
            >>> from classifiers import LocalClassifierPerParentNode, Explainer
    ...
    def explain(self, X):
        ...
        Returns
            explanation (xarray.Dataset) : An xarray.Dataset object representing the explanations for each sample
                passed.
        """
    ...
    def _explain_with_xr(self, X):
        ...
        Parameters
            X (array-like) : Sample data for which to generate SHAP values.

        Returns
            explanation (xarray.Dataset) : An xarray Dataset consisting of SHAP values for each sample.
        """
    ...
    def _get_traversed_nodes_lcppn(self, samples):
        ...
        Parameters
            samples (array-like) : Sample data for which to generate traversed nodes.

        Returns
            traversals (list) : A list of all traversed nodes as per LocalClassifierPerParentNode (LCPPN) strategy.
        """
    ...
    def _get_traversed_nodes_lcpn(self, samples):
        ...
        Returns
            traversals (list) : A list of all traversed nodes as per LocalClassifierPerNode (LCPN) strategy.
        """
        traversals = np.empty(
            (samples.shape[0], self.hierarchical_model.max_levels_),
            dtype=self.hierarchical_model.dtype_,
    ...
    def _get_traversed_nodes_lcpl(self, samples):
        ...
        Returns
            traversals (list) : A list of all traversed nodes as per LocalClassifierPerLevel (LCPL) strategy.
        """
        traversals = []
        predictions = self.hierarchical_model.predict(samples)
    ...
    def _calculate_shap_values(self, X):
        ...
        Parameters
            X (array-like) : Data for single sample for which to generate SHAP values.

        Returns
            explanation (xarray.Dataset) : A single explanation for the prediction of given sample.
        """
        traversed_nodes = []
