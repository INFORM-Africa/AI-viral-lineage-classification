...
Implements the training policies for the hierarchical classifiers as described in the referenced paper.
Silla, C.N., Freitas, A.A. A survey of hierarchical classification across different application domains. 
Data Min Knowl Disc 22, 31-72 (2011). https://doi.org/10.1007/s10618-010-0175-9
"""
...
class BinaryPolicy(ABC):
    ...
    def positive_examples(self, node) -> np.ndarray:
        ...
        Returns
            examples (np.ndarray) : A mask for which examples are included (True) and which are not.
        """
    ...
    def negative_examples(self, node) -> np.ndarray:
        ...
        Returns
            negative_examples (np.ndarray) : A mask for which examples are included (True) and which are not.
        """
        raise NotImplementedError
    ...
    def _get_descendants(self, node, inclusive: bool = True):
        ...
        Parameters
            node : Node for which the descendants should be obtained.
            inclusive (bool, default=True) : True if the given node should be included in the list of descendants.

        Returns
            descendants (set) : Set of descendants for a given node.
        """
        descendants = set()
        if inclusive:
    ...
    def _get_siblings(self, node):
        ...
        Parameters
            node : Node for which the siblings should be obtained.

        Returns
            siblings (set) : Set of siblings for a given node.
        """
...
class ExclusivePolicy(BinaryPolicy):
    ...
    def positive_examples(self, node) -> np.ndarray:
        ...
        Returns
            examples (np.ndarray) : A mask for which examples are included (True) and which are not.
        """
        examples = np.isin(self.y, node).any(axis=self._get_axis())
        return examples
    ...
    def negative_examples(self, node) -> np.ndarray:
        ...
        Returns
            negative_examples (np.ndarray) : A mask for which examples are included (True) and which are not.
        """
        examples = np.logical_not(self.positive_examples(node))
        return examples
...
class ExclusiveSiblingsPolicy(ExclusivePolicy):
    ...
    def negative_examples(self, node) -> np.ndarray:
        ...
        examples = np.isin(self.y, list(siblings)).any(axis=self._get_axis())
        return examples
...
class InclusivePolicy(BinaryPolicy):
    ...
    def positive_examples(self, node) -> np.ndarray:
        ...
        """
        descendants = self._get_descendants(node, inclusive=True)
        examples = np.isin(self.y, list(descendants)).any(
            axis=self._get_axis()
        )
        return examples
...
class SiblingsPolicy(InclusivePolicy):
    ...
    def negative_examples(self, node) -> np.ndarray:
        ...
        examples = np.isin(self.y, list(negative_classes)).any(
            axis=self._get_axis()
        )
        return examples
