...
def make_leveled(y):
    ...
    Returns
        leveled_y (array-like of shape (n_samples, n_levels)) : The leveled target values, i.e., hierarchical class
    ...
    Examples
        >>> from classifiers.HierarchicalClassifier import make_leveled
        >>> y = [['a'], ['b', 'c']]
        >>> make_leveled(y)
        array([['a', ''],
        ['b', 'c']])
...
class HierarchicalClassifier(ABC):
    ...
    def __init__(
        ...
        Parameters local_classifier (BaseEstimator, default=LogisticRegression) : The local_classifier used to create
            ...
            classifier_abbreviation (str, default="") : The abbreviation of the local hierarchical classifier to be
    ...
    def _save_tmp(self, name, classifier):
        if self.tmp_dir:
            ...
            with open(filename, "wb") as file:
                ...
                self.logger_.info(
                    f"Stored trained model for local classifier {str(name).split(self.separator_)[-1]} "
                    f"in file {filename}"
                )
