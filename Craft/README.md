...
Craft is an alignment-free machine learning pipeline for viral lineage classification. Train custom subtyping models or run predictions with pretrained models from the command line.
...
1. Navigate to the Craft source directory:
```bash
cd Craft/src
```
...
An example toml file ```dengue_standard.toml``` has been provided in the ```Craft/settings``` folder. Refer to the comments within the example toml for documentation regarding the different settings options.
...
For example (from ```Craft/src```):
```bash
python run_training.py ../settings dengue_standard.toml
```

The path to the training data must be specified in the toml settings. Training data is expected in csv format with required columns:
...
The path to the sequences must be specified in the toml settings. The sequence data is expected in fasta format.

## Running an Example

The standard dengue Craft model is provided in ```Craft/models``` and can be used to classify complete dengue sequences.

An example fasta file of publicly available dengue sequences is in ```Craft/example_input/dengue_sample.fasta```.

The template ```dengue_standard.toml``` in ```Craft/settings``` is set up to run the example fasta through the provided Craft model.
...
This writes Craft predictions for the sample sequences to ```predictions/dengue_sample_BCGR_128_craft_predictions.csv``` (relative to the working directory configured in your settings).

## Running Individual Scripts

The provided ```run_training.py``` and ```run_prediction.py``` scripts automate the preprocessing, feature extraction and modelling pipelines. To run processes individually, refer to the python scripts in the ```Craft/src/scripts``` directory.
